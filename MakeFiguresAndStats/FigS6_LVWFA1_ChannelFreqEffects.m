%% Figure S6 for White, Palmer, Boynton & Yeatman, PNAS 2019
% Makes a bar plot of the effects of each word's lexical frequency on each
% spatial channel's response, in each cue condition, in left hemisphere
% VWFA-1. 
% 
% by Alex L. White at the University of Washington, 2019
%
function FigS6_LVWFA1_ChannelFreqEffects()

%% set paths
p = getPaths(); 

%% load data
resF = fullfile(p.results, 'AllSubjChannelResponses_WordFreq.mat');
load(resF);
...
for kii=1:nChann
    ...
    subplot(nRows,nCols,kii); hold on;
...
figTitle = fullfile(p.figures,sprintf('FigS6_LeftVWFA1_FrequencyEffects.eps'));
