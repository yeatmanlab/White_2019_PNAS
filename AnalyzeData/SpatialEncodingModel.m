%% function SpatialEncodingModel() 
% Analyzes data for White, Palmer, Boynton & Yeatman, PNAS 2019
% Specifically, this function runs the spatial encoding model on each ROI in each subject. 
%
...
% AllSubjChannelResponses.mat, in the 'data' folder. 
% 
...
mainCondsToExtract = {'resp_distributedCue','resp_focalCueLeft','resp_focalCueRight'};
    ...
    for ai=1:nAreas
        for hi=1:nHems
            ...
            if sum(voxelRows)>0 %only run model if this subject has this ROI 
                ...
                for ci=1:nAttnConds
                    eval(sprintf('D(:,ci) = T.%s(voxelRows);', mainCondsToExtract{ci}));
                end
...
cueCondLabels = cell(size(mainCondsToExtract)); 
cueCondLabels(strcmp(mainCondsToExtract,'resp_distributedCue')) = {'Distributed cue'};
cueCondLabels(strcmp(mainCondsToExtract,'resp_focalCueLeft')) = {'Focal cue left'};
cueCondLabels(strcmp(mainCondsToExtract,'resp_focalCueRight')) = {'Focal cue right'};
...
%% save
%save this file 
resFileName = fullfile(p.results,'AllSubjChannelResponses.mat'); 
